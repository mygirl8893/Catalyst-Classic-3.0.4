...
void InfoDialog::timerEvent(QTimerEvent* _event) {
  if (_event->timerId() == m_refreshTimerId) {
    ...
    quint64 Incoming =    NodeAdapter::instance().getIncomingConnectionsCount();
        m_ui->m_connections->setText(QString(tr("%1 (Send: %2, Incoming: %3)")).arg(Connections).arg(Outgoing).arg(Incoming));
    ...
    quint64 greyPeerList = NodeAdapter::instance().getGreyPeerlistSize();
        m_ui->m_peerList->setText(QString(tr("White: %1, gray: %2")).arg(whitePeerList).arg(greyPeerList));
    ...
    quint64 lastLocalBlockHeight = NodeAdapter::instance().getLastLocalBlockHeight();
        m_ui->m_height->setText(QString(tr("Remote: %1, local: %2")).arg(lastKnownBlockHeight).arg(lastLocalBlockHeight));

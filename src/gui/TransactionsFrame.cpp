...
TransactionsFrame::TransactionsFrame(QWidget* _parent) : QFrame(_parent), m_ui(new Ui::TransactionsFrame),
  ...
  contextMenu->addAction(QString(tr("duplicate &amount")), this, SLOT(copyAmount()));
  contextMenu->addAction(QString(tr("Indicated &details")), this, SLOT(showTxDetails()));
...
void TransactionsFrame::exportToCsv() {
  ...
  if (!file.isEmpty()) {
    QByteArray res;
    res.append("\"State\",\"date\",\"amount\",\"cost\",\"hash\",\"height\",\"address\",\"payment ID\"\n");
